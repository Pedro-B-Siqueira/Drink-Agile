...
def view_histforn():
    try:
        ...
        selectFunc = "SELECT * FROM historicos_fornecedores"
        cursor.execute(selectFunc)

        print("{:<30} {:<30} {:<30} {:<30} {:<50} {:<30} {:<30} {:<50}".format('ID HISTORICO', 'ID FORNECEDOR', 'NOME(OLD)', 'CNPJ(OLD)', 'ENDEREÇO(OLD)','NOME(NEW)', 'CNPJ(NEW)', 'ENDEREÇO(NEW)',))
